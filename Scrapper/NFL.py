...
class NFLScrapper:
    ...
    def getNFLData(self):
        ...
        for year in range(self.initial_year, self.final_year + 1): 
            try:
...
if(__name__ == "__main__"):
    ...
    parser.add_argument('-a', '--attribute', required = False, help = 'Spreadsheet status that you want to collect', default = 'Passing' )
    parser.add_argument('-st', '--seasontype', required = False, help = 'Part of the season that you want to collect', default = 'REG')
    
    args = parser.parse_args()
    
    NFL = NFLScrapper(int(args.initial_year), int(args.final_year), args.attribute, args.seasontype)
    data = NFL.getNFLData()
    NFL.SaveData(data)
